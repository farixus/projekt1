...
def scan_hosts(ips: list) -> dict:
    ...
    for ip in ips:
        ...
        for socket_to_check in sockets_to_check:
            if socket_state(socket_to_check) != '':
                ip,port = socket_to_check.split('/')
                print(f'Socket {socket_to_check} is opened')
                open_ports.append(port)
            else:   #prints out closed ports
                print(f'Socket {socket_to_check} is closed.')
